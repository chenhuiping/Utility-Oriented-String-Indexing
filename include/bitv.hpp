...
class bitv {
   public:
    ...
    uint64_t bit_size() { return 8 * sizeof(this) + ones.size() * sizeof(uint64_t) * 8; }
   ...
   private:
    ...
    class pred_search {
        class ps_iterator : public std::iterator<random_access_iterator_tag, bool> {
            ...
            ps_iterator(pred_search *v, uint64_t index) : _ps(v), _index(index) {}
           ...
           public:
            ...
            bool operator*() const { return (*_ps)[_index]; }
            ...
            bool operator==(ps_iterator it) const { return _index == it._index; }
            ...
            friend ps_iterator operator+(uint64_t n, ps_iterator it) { return it + n; }
            ...
            uint64_t operator-(ps_iterator it) { return uint64_t(_index) - uint64_t(it._index); }
