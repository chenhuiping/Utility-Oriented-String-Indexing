...
double get_utility(LRUCache &LRU, vector<unsigned char> &pattern, vector<double> &PS, unsigned char *sequence, INT *SA, INT *LCP, rmq_succinct_sct<> &rmq, INT n) {
    INT fp = 0;
...
int main(int argc, char *argv[]) {
    ...
    while (is2.read(reinterpret_cast<char *>(&c), 1)) {
        if (c == '\n') {
            ...
            get_utility(LRU, pattern, PS, sequence, SA, LCP, rmq, n);
            pattern_num++;
            pattern.clear();
        } else
